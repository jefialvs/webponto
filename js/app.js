const STORAGE_KEY = 'controle_ponto_campo_final_ok_v4';

const OPERADORES_PRE_CADASTRADOS = {
  jhon: {
    nome: 'Jhon Leno Queiroz da Silva',
    codigo: '23082022'
  },
  jeferson: {
    nome: 'Jeferson Alves de Brito',
    codigo: '25112025'
  }
};
...
const operadorSelect = byId('operadorSelect');
...
const printAssinaturaOperadorRodape = byId('printAssinaturaOperadorRodape');
const printAssinaturaResponsavelRodape = byId('printAssinaturaResponsavelRodape');
...
function iniciar() {
  ...
  if (empresaNomeInput) empresaNomeInput.value = 'Agua Pura';
  if (empresaCnpjInput) empresaCnpjInput.value = '02.809.871/0001-86';
  if (responsavelNomeInput) responsavelNomeInput.value = 'Diego';
  if (cidadeUfInput && !cidadeUfInput.value) cidadeUfInput.value = 'Manaus/AM';
  if (metaDiariaInput) metaDiariaInput.value = '09:00';
  ...
  operadorSelect?.addEventListener('change', aplicarOperadorPreCadastrado);
...
function aplicarOperadorPreCadastrado() {
  ...
  codigoOperadorInput.value = operador.codigo;
}
...
function salvarRegistro(evento) {
  ...
  const registro = {
    ...
    empresaNome: valor(empresaNomeInput) || 'Agua Pura',
    empresaCnpj: valor(empresaCnpjInput) || '02.809.871/0001-86',
    responsavelNome: valor(responsavelNomeInput) || 'Diego',
    ...
    cidadeUf: valor(cidadeUfInput) || 'Manaus/AM',
    ...
    metaDiaria: normalizarHora(metaDiariaInput?.value) || '09:00',
...
function limparFormulario() {
  ...
  if (empresaCnpjInput) empresaCnpjInput.value = '02.809.871/0001-86';
  if (responsavelNomeInput) responsavelNomeInput.value = 'Diego';
  if (cidadeUfInput) cidadeUfInput.value = 'Manaus/AM';
  if (metaDiariaInput) metaDiariaInput.value = '09:00';
...
function editarRegistro(id) {
  ...
  if (empresaNomeInput) empresaNomeInput.value = r.empresaNome || 'Agua Pura';
  if (empresaCnpjInput) empresaCnpjInput.value = r.empresaCnpj || '02.809.871/0001-86';
  if (responsavelNomeInput) responsavelNomeInput.value = r.responsavelNome || 'Diego';
  ...
  if (cidadeUfInput) cidadeUfInput.value = r.cidadeUf || 'Manaus/AM';
  ...
  if (metaDiariaInput) metaDiariaInput.value = r.metaDiaria || '09:00';
  ...
  if (operadorSelect) {
    ...
    if (r.codigoOperador === '23082022') operadorSelect.value = 'jhon';
    if (r.codigoOperador === '25112025') operadorSelect.value = 'jeferson';
  }
...
function calcularRegistro(registro) {
  const metaMinutos = converterHoraParaMinutos(registro.metaDiaria || '09:00');
...
function renderizarRelatorioImpressao(lista) {
  try {
    ...
    if (printEmpresaNome) printEmpresaNome.textContent = primeiro.empresaNome || 'Agua Pura';
    if (printEmpresaCnpj) printEmpresaCnpj.textContent = `CNPJ: ${primeiro.empresaCnpj || '02.809.871/0001-86'}`;
    if (printPeriodo) {
    ...
    if (printTurnoBase) printTurnoBase.textContent = primeiro.turnoBase || '07:30 / 12:00 / 13:00 / 17:30';
    if (printMetaDiaria) printMetaDiaria.textContent = primeiro.metaDiaria || '09:00';
    if (printAssinaturaOperador) printAssinaturaOperador.textContent = operadorPrincipal;
    if (printAssinaturaResponsavel) printAssinaturaResponsavel.textContent = primeiro.responsavelNome || 'Diego';
    if (printAssinaturaOperadorRodape) printAssinaturaOperadorRodape.textContent = operadorPrincipal;
    if (printAssinaturaResponsavelRodape) printAssinaturaResponsavelRodape.textContent = primeiro.responsavelNome || 'Diego';

    if (!lista.length) {
      printTabela.innerHTML = `<tr><td colspan="12" style="text-align:center;padding:12px;">Nenhum registro para impressão.</td></tr>`;
    } else {
